...
from localpackage.modified_ozone.urls import URLs
...
def parse_incoming_result(json_object: dict) -> pandas.DataFrame:
    ...
    for spec in OUTPUT["species"]:
        ...
        for step in spec["values"]:
            ...
            date = datetime.fromtimestamp(step["t"]["d"])-relativedelta(days=1)
            value: int = step["v"]
...
def get_data_from_id(city_id: int):
    ...
    result = result.sort_values(by=['pm25'], ascending=False, na_position="last")
...
class Ozone:
    ...
    def get_city_forecast(
        ...
        return self._format_output(data_format, df)
    def get_city_id(self,lat,lon):
        
        r = self._make_api_request(
            f"{self._search_aqi_url}/geo:{lat};{lon}/?token={self.token}"
        )
        city = self._check_and_get_data_obj(r)['idx']
        return city
